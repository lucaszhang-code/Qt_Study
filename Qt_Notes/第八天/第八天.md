...
在之前的`tableView`中，表格内容只能通过输入的形式修改，但是对于某些信息，比如数字，我们希望使用`spinBox`修改，对于某些固定选择的信息，我们希望通过下拉菜单的形式修改

因此，我们可以自定义代理，更改原本`tableView`的表格组件某些特性，满足用户的需求
...
一般会将`QObject`和`Q_OBJECT`勾选，并且自定义的类名用首写字母T表示，这个类继承自`QStyledItemDelegate`

![勾选信息](./assets/勾选信息.png)
...
TSpinBoxDelegate::TSpinBoxDelegate()
{
    
}
```

我们最好在`new`这个类的时候将他与父类绑定，这样可以把它挂载到对象树中，不用手动管理内存
...
#### 2.右键类名，点击重构，选择“为基础类插入虚函数”

选择`QStyledItemDelegate`类下面的`createEditor`,`setEditorData`,`setModelData`,`updateEditor`虚函数

下方两个勾选框可以勾上，下拉菜单选择如下

![插入虚函数](./assets/插入虚函数.png)

#### 3.编写虚函数

IDE会自动为你生成定义和相应的虚函数，但是我们通常将函数放在`cpp`文件中，因此我们将虚函数剪切到对应的`cpp`文件中进行编写\
...
- **`QWidget *parent`**:
  - 这是编辑器控件的父控件指针。通常，这 个参数是视图的指针，用于确定编辑器在界面层次结构中的位置，并确保当视图被销毁时，编辑器也会被正确地销毁。
- **`const QStyleOptionViewItem &option`**:
  - 提供有关视图项的样式选项信息，包括几何形状、状态、颜色等。可以用于自定义编辑器的样式或行为。
- **`const QModelIndex &index`**:
  - 这是一个 `QModelIndex` 对象，指示当前正在编辑的模型项的具体位置。通过 `index`，你可以访问模型中的数据，以根据特定单元格的位置自定义编辑器的行为或外观。
...
- **`QWidget *editor`**:
  - 这是 `createEditor` 函数中创建的编辑器控件的指针。
- **`const QModelIndex &index`**:
  - 这是一个 `QModelIndex` 对象，指示当前正在编辑的模型项的位置。通过 `index`，可以访问模型中的数据，并将其设置到编辑器中。
...
- **`QAbstractItemModel *model`**:
  - 这是指向视图使用的数据模型的指针。通过这个参数，你可以将用户在编辑器中修改后的数据保存回模型中。模型通常是 `QStandardItemModel` 或自定义的模型类。
- **`const QModelIndex &index`**:
  - 这是一个 `QModelIndex` 对象，指示要在模型中更新数据的位置。通过这个索引，可以指定模型中的特定单元格来更新其值。
...
- **`QWidget *editor`**:
  - 编辑的组件
- **`const QStyleOptionViewItem &option`**:
  - 提供了有关视图项几何形状和样式的信息。`option.rect` 包含编辑器应占据的矩形区域，`setGeometry(option.rect)` 用于将编辑器设置在正确的位置和大小。
- **`const QModelIndex &index`**:
  - 这是一个 `QModelIndex` 对象，指示要调整编辑器位置的模型项的位置。它可以用于在某些情况下根据单元格位置自定义编辑器几何形状。

```c++
// spinkboxdelegate.cpp
...
QWidget *TSpinkBoxDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    Q_UNUSED(option);
    Q_UNUSED(index);
    QSpinBox *editor = new QSpinBox(parent);	// 创建一个QSpinBox组件
    editor->setFrame(false);	// 不设置边框
    editor->setMinimum(0);	// 可供选择的最小值是0
    editor->setMaximum(INT_MAX);	// 可供选择的最小值是INT_MAX
    return editor;	// 返回设置的组件
}

// 将数据从模型设置到编辑器
void TSpinkBoxDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
    QSpinBox *spinBox = dynamic_cast<QSpinBox*>(editor);	// 进行组件类的转换，虽然一般情况函数传入的editor是QSPinBox，但我们最好还是做一下转换
    int value = index.model()->data(index,Qt::EditRole).toInt();	// 根据index（索引）从model中获取数据，并将他转化成整型
    spinBox->setValue(value);	// spinBox用于展示数据
}
...
void TSpinkBoxDelegate::setModelData(QWidget *editor, QAbstractItemModel *model, const QModelIndex &index) const
{
    ...
    int value = spinBox->value();	// 获取用户传递的数据
    model->setData(index, value);	// 将最新的数据通过index返回给模型进行修改
}
...
{
    Q_UNUSED(index);
    editor->setGeometry(option.rect);	// 将编辑器的几何形状设置为 option.rect
}
```

按照此方法可以重构其他类，这里不做过多展示，不过`TComboBoxDelegate`比较特殊
...
private:
    QStringList m_itemList;	// 定义一个集合用于接收下拉菜单的选项
    bool m_editable;	// 是否在编辑
public:
...
QWidget *TComboBoxDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    for(auto &item : m_itemList){	// 设置下拉菜单的选项
        editor->addItem(item);
    }
    return editor;
}
...
写在`mainwindow.cpp`的构造函数里面

```c++
 	intSpinDelegate = new TSpinkBoxDelegate(this);
    ui->tableView->setItemDelegateForColumn(0, intSpinDelegate);	// 第0列采用TSpinkBoxDelegate组件

    floatSpinDelegate = new TFloatSpinDelegate(this);
    ui->tableView->setItemDelegateForColumn(1, floatSpinDelegate);	// 第1到3列采用TFloatSpinDelegate组件
    ...
    QStringList strList;
    strList<<"优"<<"良"<<"中"<<"差";	// 定义下拉菜单的选项
    comboDelegate->setItems(strList, false);	// 调用自定义函数，传入参数
    ui->tableView->setItemDelegateForColumn(4, comboDelegate);	// 第4列采用TComboBoxDelegate组件
```

### 最终效果

![自定义代理类](./assets/自定义代理类.png)
